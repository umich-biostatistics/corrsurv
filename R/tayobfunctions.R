...
RMRL_function=function(data_format,Tau,t)
{
  ...
  list(RMRL=RMRL,area_under_RMRL=area_under_RMRL,var_area_under_RMRL=var_area_under_RMRL)
  # OUTPUT
...
two_sample_recurrent_events_test=function(X,delta,Z,Group,Tau,t,method="average",plot=FALSE)
{
  ############format observed data for our method
  # INPUT
  # X=vector containing observed time to terminating event
  ...
  formatted_group1_data=format_data_ourmethod(X=X[Group==Group_id[1]],delta=delta[Group==Group_id[1]],Z=Z[Group==Group_id[1],],t)
  formatted_group2_data=format_data_ourmethod(X=X[Group==Group_id[2]],delta=delta[Group==Group_id[2]],Z=Z[Group==Group_id[2],],t)  
  
  if(method=="average")
  {
    results1=get_mu_hat_star_tau(formatted_group1_data$X_tk,formatted_group1_data$delta_tk,Tau=Tau,t=t)
    results2=get_mu_hat_star_tau(formatted_group2_data$X_tk,formatted_group2_data$delta_tk,Tau=Tau,t=t)
  ...
  if(method=="area")
  {
    ...
    if(plot)
    {
      ...
      matplot(cbind(t,t),cbind(RMRL1,RMRL2),type="l",xaxs="i",xaxt="n",yaxs="i",xlab="Time",ylab="RMRL",xlim=c(t[1]-xadj,t[length(t)]+xadj),ylim=c(min(RMRL1,RMRL2)-yadj1,max(RMRL1,RMRL2)+yadj2))
      points(t,RMRL1,pch=20,cex=1,col="black")
      points(t,RMRL2,pch=20,cex=1,col="red")
      axis(1,at=t,labels=t)
      legend("bottomright",c("Group 1","Group 2"),col=c("black","red"),lty=c(1,2),bty="n")
    }
  }
  ...
  for(k in 1:length(t))
  {
    follow_up_windows_print=paste(follow_up_windows_print,paste("[",t[k],",",t[k]+Tau,"]"),"     ")
  }
  ...
  return(list(Mean=mean12,Var=var12,test_stat=test_stat,test_stat_p=test_stat_p))
  # OUTPUT
